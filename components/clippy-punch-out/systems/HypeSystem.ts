...
export class HypeSystem {
  update(ctx: GameContext, dt: number) {
    if (ctx.player.state.action === 'guard') {
      if (ctx.player.guardDuration > CFG.player.guard.maxComfortMs) {
  ...
  onCounterPunch(ctx: GameContext)   { this.add(ctx, CFG.hype.gains.counterPunch) }
  onCombo(ctx: GameContext)          { this.add(ctx, CFG.hype.gains.combo) }
  onTauntHit(ctx: GameContext)       { this.add(ctx, CFG.hype.gains.tauntHit) }
  ...
  onGuardBlock(ctx: GameContext)     { this.remove(ctx, CFG.hype.losses.guardBlock) }
