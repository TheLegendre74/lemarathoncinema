...
function CountdownMini({ marathonStart }: { marathonStart: string }) {
  ...
  if (!now) return null
  const start = new Date(marathonStart)
...
export default function AuthPageClient({ marathonStart, saisonLabel, saisonNumero }: {
  ...
  const live = new Date() >= new Date(marathonStart)
