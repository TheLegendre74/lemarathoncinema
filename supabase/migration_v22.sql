-- Migration v22 : règles du jeu éditables depuis l'admin + colonne tutorial profil

-- Pré-remplir MARATHON_RULES avec le contenu structuré en JSON
...
WHERE key = 'MARATHON_RULES';

-- Si la ligne n'existe pas encore (sécurité)
INSERT INTO public.site_config (key, value)
SELECT 'MARATHON_RULES', '[]'
WHERE NOT EXISTS (SELECT 1 FROM public.site_config WHERE key = 'MARATHON_RULES');

-- Colonne tutorial_seen pour afficher le tuto une seule fois aux nouveaux joueurs
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS tutorial_seen bool NOT NULL DEFAULT false;
